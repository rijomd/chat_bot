        ...
        <div className="w-full md:w-1/3 bg-white overflow-y-auto ">
            {myList?.length > 0 && myList.map((item, index) => (
                <div
                    ...
                    key={index}
                    className={`p-4 hover:bg-green-50 cursor-pointer flex items-center gap-3 transition-colors  ${
                        selectedUser?.id === item?.id ? 'bg-green-100 border-l-4 border-green-500' : ''
                    }`}
                >
